...
fn main() {
    ...
    let mut local_button = Button::new("Files", button_metrics)
        .with_colors(
            theme.button,
            theme.button_hover,
            theme.button_active,
    ...
    let mut remote_button = Button::new("Remote", button_metrics)
        .with_colors(
            [0.90, 0.55, 0.20],
            [0.95, 0.70, 0.30],
            [0.85, 0.45, 0.10],
    ...
    local_button.set_on_click(move || {
        *status_local.borrow_mut() = "Opening local panel...".to_string();
    });
